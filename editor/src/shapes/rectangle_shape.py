...
class RectangleShape(Shape):
    ...
    def copy(self, copy_name=False):
        newob = RectangleShape(self.anchor_at.copy(), self.border_color.copy(), self.border_width,
                        self.fill_color.copy(), self.width, self.height, self.corner_radius)
        self.copy_into(newob, copy_name)
        return newob

    def draw_path(self, ctx, for_fill=False):
        Shape.rounded_rectangle(ctx, 0, 0, self.width, self.height, self.corner_radius)
