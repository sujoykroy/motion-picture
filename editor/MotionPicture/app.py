...
class ApplicationWindow(MasterEditor):
    ...
    def create_new_shape(self, action, parameter):
        ...
        if shape_type == "image":
            ...
            filename = FileOp.choose_file(self, purpose="open", file_types=[["Document", "*.xml"]])
            if not filename:
                return
    ...
    def break_points_into_point_shapes(self, action, parameter):
        if self.shape_manager.break_points_into_point_shapes():
            self.redraw()
